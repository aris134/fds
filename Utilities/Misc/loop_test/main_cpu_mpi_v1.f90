PROGRAM LOOP3D
    ...
    REAL(EB) :: FLUX_AVGS(3)
    INTEGER :: MY_BBOX(6)
    INTEGER, ALLOCATABLE, DIMENSION(:) :: HALO_SEND_SIZES, HALO_SEND_DISPLS
    INTEGER, ALLOCATABLE, DIMENSION(:) :: HALO_RECV_SIZES, HALO_RECV_DISPLS ! num points received by each neighbor
    ...
    REAL(EB), ALLOCATABLE, DIMENSION(:) :: OMX_SEND, OMY_SEND, OMZ_SEND, TXZ_SEND, TXY_SEND, TYZ_SEND
    REAL(EB), ALLOCATABLE, DIMENSION(:) :: OMX_RECV, OMY_RECV, OMZ_RECV, TXZ_RECV, TXY_RECV, TYZ_RECV
    ...
    INTEGER :: LOC_MIN_I, LOC_MIN_J, LOC_MIN_K, LOC_MAX_I, LOC_MAX_J, LOC_MAX_K, LOC_MAX_I_P1, LOC_MAX_J_P1, LOC_MAX_K_P1, &
               LOC_MIN_I_P1, LOC_MIN_J_P1, LOC_MIN_K_P1, LOWER_K, LOC_MAX_I_P2, LOC_MAX_J_P2, LOC_MAX_K_P2
...
TYPE(EDGE_TYPE), ALLOCATABLE, DIMENSION(:) :: EDGE
    ...
    LOC_MAX_I_P2 = LOC_MAX_I + 20
    LOC_MAX_J_P2 = LOC_MAX_J + 20
    LOC_MAX_K_P2 = LOC_MAX_K + 20
    ...
    END IF
        ...
        ! Set the halo maximum values
    IF (LOC_MAX_I + 2*HALO_DEPTH <= GLOB_MAX_I) THEN
        LOC_MAX_HALO_I_P1 = LOC_MAX_I + 2*HALO_DEPTH
    ELSE
        LOC_MAX_HALO_I_P1 = GLOB_MAX_I
    END IF

    IF (LOC_MAX_J + 2*HALO_DEPTH <= GLOB_MAX_J) THEN
        LOC_MAX_HALO_J_P1 = LOC_MAX_J + 2*HALO_DEPTH
    ELSE
        LOC_MAX_HALO_J_P1 = GLOB_MAX_J
    END IF

    IF (LOC_MAX_K + 2*HALO_DEPTH <= GLOB_MAX_K) THEN
        LOC_MAX_HALO_K_P1 = LOC_MAX_K + 2*HALO_DEPTH
    ELSE
        LOC_MAX_HALO_K_P1 = GLOB_MAX_K
    END IF
    ...
    IF (LOC_MIN_I - 2*HALO_DEPTH >= GLOB_MIN_I) THEN
        LOC_MIN_HALO_I_M1 = LOC_MIN_I - 2*HALO_DEPTH
    ELSE
        LOC_MIN_HALO_I_M1 = GLOB_MIN_I
    END IF

    IF (LOC_MIN_J - 2*HALO_DEPTH >= GLOB_MIN_J) THEN
        LOC_MIN_HALO_J_M1 = LOC_MIN_J - 2*HALO_DEPTH
    ELSE
        LOC_MIN_HALO_J_M1 = GLOB_MIN_J
    END IF

    IF (LOC_MIN_K - 2*HALO_DEPTH >= GLOB_MIN_K) THEN
        LOC_MIN_HALO_K_M1 = LOC_MIN_K - 2*HALO_DEPTH
    ELSE
        LOC_MIN_HALO_K_M1 = GLOB_MIN_K
    END IF
    ...
    ALLOCATE(X(LOC_MIN_HALO_I_M1:LOC_MAX_HALO_I_P1), Y(LOC_MIN_HALO_J_M1:LOC_MAX_HALO_J_P1), Z(LOC_MIN_HALO_K_M1:LOC_MAX_HALO_K_P1))
    ALLOCATE(RDXN(LOC_MIN_HALO_I_M1:LOC_MAX_HALO_I_P1), RDYN(LOC_MIN_HALO_J_M1:LOC_MAX_HALO_J_P1), RDZN(LOC_MIN_HALO_K_M1:LOC_MAX_HALO_K_P1))
    ALLOCATE(RDX(LOC_MIN_HALO_I_M1:LOC_MAX_HALO_I_P1), RDY(LOC_MIN_HALO_J_M1:LOC_MAX_HALO_J_P1), RDZ(LOC_MIN_HALO_K_M1:LOC_MAX_HALO_K_P1))
    ALLOCATE(U(LOC_MIN_HALO_I_M1:LOC_MAX_I_P2,LOC_MIN_HALO_J_M1:LOC_MAX_J_P2,LOC_MIN_HALO_K_M1:LOC_MAX_K_P2))
    ALLOCATE(V(LOC_MIN_HALO_I_M1:LOC_MAX_I_P2,LOC_MIN_HALO_J_M1:LOC_MAX_J_P2,LOC_MIN_HALO_K_M1:LOC_MAX_K_P2))
    ALLOCATE(W(LOC_MIN_HALO_I_M1:LOC_MAX_I_P2,LOC_MIN_HALO_J_M1:LOC_MAX_J_P2,LOC_MIN_HALO_K_M1:LOC_MAX_K_P2))
    ALLOCATE(MU(LOC_MIN_I:LOC_MAX_I_P2,LOC_MIN_J:LOC_MAX_J_P2,LOC_MIN_K:LOC_MAX_K_P2))!didn't have halo in max terms (but had p1)
    ...
    IF (MY_RANK==7) WRITE(*,*) 'MY_RANK=',MY_RANK,' FINISHED ALLOCATE..'

    ! Initialize:
    DO I=LOC_MIN_HALO_I_M1,LOC_MAX_HALO_I_P1
       X(I)    = REAL(I,EB)
       RDXN(I) = 1.0_EB
       RDX(I)  = 1.0_EB
    ENDDO
    DO J=LOC_MIN_HALO_J_M1,LOC_MAX_HALO_J_P1
       Y(J)    = REAL(J,EB)
       RDYN(J) = 1.0_EB
       RDY(J)  = 1.0_EB
    ENDDO
    DO K=LOC_MIN_HALO_K_M1,LOC_MAX_HALO_K_P1
       Z(K)    = REAL(K,EB)
       RDZN(K) = 1.0_EB
       RDZ(K)  = 1.0_EB
    ENDDO
    ...
    IF (LOC_MIN_K > GLOB_MIN_K) THEN
        LOWER_K = LOC_MIN_HALO_K
    ELSe
        LOWER_K = LOC_MIN_K_P1
    END IF
    ...
    CELL_INDEX = 0
    IC = 0
    DO K=LOC_MIN_K_P1,LOC_MAX_HALO_K ! replace loc_min_k_p1 with loc_min_halo_k
       DO J=LOC_MIN_J_P1,LOC_MAX_HALO_J
          DO I=LOC_MIN_I_P1,LOC_MAX_HALO_I
            IF( .NOT. (ANY( K==(/GMINKP1,GLOB_MAX_K/) ) .OR. ANY( J==(/GMINJP1,GLOB_MAX_J/) ) .OR. ANY( I==(/GMINIP1,GLOB_MAX_I/) )) ) CYCLE
             ...
             CELL_INDEX(I,J,K) = IC
             !IF (MY_RANK==0) WRITE(*,*) 'I-J-K=',I,'-',J,'-',K,' and IC=',IC
          ENDDO
       ENDDO
    ENDDO
    ALLOCATE(CELL(0:IC))
    
    WRITE(*,*) 'MY_RANK=',MY_RANK,' STARTED FILLING CELL..'
    IC = 0
    MAX_EDGE=-1
    DO K=LOC_MIN_K_P1,LOC_MAX_K!this was loc_max_halo_k
       DO J=LOC_MIN_J_P1,LOC_MAX_J!this was loc_max_halo_j
          DO I=LOC_MIN_I_P1,LOC_MAX_I!this was loc_max_halo_i
            IF( .NOT. (ANY( K==(/GMINKP1,GLOB_MAX_K/) ) .OR. ANY( J==(/GMINJP1,GLOB_MAX_J/) ) .OR. ANY( I==(/GMINIP1,GLOB_MAX_I/) )) ) CYCLE
             ! IC = IC + 1
             IC = IC_VAL(I,J,K,GLOB_MAX_I,GLOB_MAX_J,GLOB_MAX_K)
             ...
             CELL(IC)%EDGE_INDEX(6)  = CELL_INDEX(I  ,J  ,K-1) + 3 ! is this term causing an issue?
             CELL(IC)%EDGE_INDEX(7)  = CELL_INDEX(I  ,J  ,K  ) + 4
             CELL(IC)%EDGE_INDEX(8)  = CELL_INDEX(I  ,J+1,K  ) + 4
             CELL(IC)%EDGE_INDEX(9)  = CELL_INDEX(I  ,J  ,K  ) + 5
             CELL(IC)%EDGE_INDEX(10) = CELL_INDEX(I  ,J  ,K-1) + 5 ! or is this term causing an issue? (is cell index defined when K = 0???)
       ...
       ENDDO
    ENDDO
    WRITE(*,*) 'MY_RANK=',MY_RANK,' FINISHED FILLING CELL..'
    ...
    ! U, V, W: determine velocity components at the halo regions locally
    DO K=LOC_MIN_HALO_K_M1,LOC_MAX_HALO_K_P1
       DO J=LOC_MIN_HALO_J_M1,LOC_MAX_HALO_J_P1
          DO I=LOC_MIN_HALO_I_M1,LOC_MAX_HALO_I_P1
             ! Some Trig functions:
       ...
       ENDDO
    ENDDO
    WRITE(*,*) 'MY_RANK=',MY_RANK,' FINISHED INIT VELOCITY..'
    ! Compute Tau OMG:
    UU => U
    VV => V
    WW => W
    ...
    WRITE(*,*) 'MY_RANK=',MY_RANK,' MADE IT PAST TEST BARRIER AND LOC_MAX_HALO_J+1=',LOC_MAX_HALO_J+1,' LOC_MAX_J_P2=',LOC_MAX_J_P2
   
    DO K=LOC_MIN_HALO_K,LOC_MAX_HALO_K
       DO J=LOC_MIN_HALO_J,LOC_MAX_HALO_J
          DO I=LOC_MIN_HALO_I,LOC_MAX_HALO_I
             ...
             OMX(I,J,K) = DWDY - DVDZ
             !IF (MY_RANK==0 .AND. I == 128 .AND. J == 256 .AND. K == 256) WRITE(*,*) 'RANK 0 OMX AT IJK = 128,256,256 = ',OMX(I,J,K)
             OMY(I,J,K) = DUDZ - DWDX
             OMZ(I,J,K) = DVDX - DUDY
             MUX = 0.25_EB*4*0.0019_EB!(MU(I,J+1,K)+MU(I,J,K)+MU(I,J,K+1)+MU(I,J+1,K+1)) ! have a seg fault somewhere on MU array
             MUY = 0.25_EB*4*0.0019_EB!(MU(I+1,J,K)+MU(I,J,K)+MU(I,J,K+1)+MU(I+1,J,K+1))
             MUZ = 0.25_EB*4*0.0019_EB!(MU(I+1,J,K)+MU(I,J,K)+MU(I,J+1,K)+MU(I+1,J+1,K))
    ...
    ENDDO
    CALL MPI_Barrier(MPI_COMM_WORLD, IERR)
    WRITE(*,*) 'MY_RANK=',MY_RANK,' FINISH INIT VORTICITY..'


    !TODO: MPI exchange to figure out what OMX, OMY, etc. are at the halos
    !TODO: first, determine the point indexes that we need to send
    !TODO: we need to figure out how many points we're sending to each nbr
    WRITE(*,*) 'MY_RANK=',MY_RANK,' STARTING DETERMINE HALO SEND SIZE..'
    ...
    !WRITE(*,*) 'MY_RANK=',MY_RANK,' NUM_NBRS=',NUM_NBRS,' HALO_RECV_DISPLS(1)=',HALO_RECV_DISPLS(1),' HALO_SEND_DISPLS(1)=',HALO_SEND_DISPLS(1)
    ...
    !IF (MY_RANK==0) WRITE(*,*) 'RANK 0 RIGHT BEFORE DETERMINE_DATA_TO_EXCHANGE: OMX(128,256,256) = ',OMX(128,256,256)
    !CALL DETERMINE_DATA_TO_EXCHANGE(NUM_NBRS,HALO_SEND_DISPLS,NBR_EXT_BBOXES,OMX_SEND,&
    !                                OMY_SEND,OMZ_SEND,TXZ_SEND,TXY_SEND,TYZ_SEND,OMX,&
    !                                OMY,OMZ,TXZ,TXY,TYZ,BOX_SIZE,SEND_COORDS,MY_RANK)
    !IF (MY_RANK==0) WRITE(*,*) 'RANK 0 RIGHT AFTER DETERMINE_DATA_TO_EXCHANGE: SEND_COORDS(1:3)=',SEND_COORDS(1),',',SEND_COORDS(2),',',SEND_COORDS(3)

    ! Verify sizes were exchanged correctly
    !WRITE(*,*) 'MY_RANK=',MY_RANK,' HALO_RECV_SIZES(1)=',HALO_RECV_SIZES(1),' HALO_SEND_SIZES(1)=',HALO_SEND_SIZES(1)


    ! At this point, we have the halos setup, and the halo message sizes, displacements
    ! We just need to fill the actual data arrays and then exchange them

    !TODO: MPI EXCHANGE HALO SEND SIZES TO OBTAIN HALO RECEIVE SIZES
    WRITE(*,*) 'RANK=',MY_RANK,' STARTING EXCHANGE_HALO_DATA'
    !CALL EXCHANGE_HALO_DATA(OMX_SEND,OMX_RECV,OMY_SEND,OMY_RECV,OMZ_SEND,OMZ_RECV,&
    ...
    WRITE(*,*) 'RANK=',MY_RANK,' FINISHED EXCHANGE_HALO_DATA'

    !TODO: Setup communicated data 
    !TODO CALL SETUP_HALO_DATA
    WRITE(*,*) 'RANK=',MY_RANK,' STARTING SETUP_HALO_DATA'
    !CALL SETUP_HALO_DATA(NUM_NBRS,HALO_RECV_SIZES,HALO_RECV_DISPLS,RECV_COORDS,&
    !                     OMX_RECV,OMY_RECV,OMZ_RECV,TXZ_RECV,TXY_RECV,TYZ_RECV,&
    !                     OMX,OMY,OMZ,TXZ,TXY,TYZ,MY_RANK)
    WRITE(*,*) 'RANK=',MY_RANK,' FINISHED SETUP_HALO_DATA'
    ...
    !WRITE(*,*) 'MY_RANK=',MY_RANK,'SUM FVX=',FLUX_AVGS(1)
...
!$OMP DO SCHEDULE(STATIC) PRIVATE(WOMY, VOMZ, TXXP, TXXM, DTXXDX, DTXYDY, DTXZDZ)
    ...
    SUBROUTINE EXCHANGE_HALO_DATA(OMX_SEND,OMX_RECV,OMY_SEND,OMY_RECV,OMZ_SEND,OMZ_RECV,&
        ...
        CALL EXCHANGE_INTEGER_DATA(SEND_COORDS,RECV_COORDS,NUM_NBRS,3*HALO_SEND_SIZES,SEND_COORD_DISPLS,3*HALO_RECV_SIZES,RECV_COORD_DISPLS,MY_RANK)
    ...
    SUBROUTINE EXCHANGE_INTEGER_DATA(SEND_DATA, RECV_DATA, NUM_NBRS, SEND_SIZES, SEND_DISPLS,&
                                     RECV_SIZES, RECV_DISPLS,MY_RANK)
        ...
        DO K=LOC_MIN_K_P1,LOC_MAX_K
           DO J=LOC_MIN_J_P1,LOC_MAX_J
              DO I=LOC_MIN_I,LOC_MAX_I
                 ...
                 VTRM  = DTXXDX + DTXYDY + DTXZDZ
                 FVX(I,J,K) = 0.25_EB*(WOMY - VOMZ) - GX(I) + RRHO*(GX(I)*RHO_0(K) - VTRM)
                 !WRITE(*,*) 'FVX(',I,',',J,',',K,')=',FVX(I,J,K)
                 !IF (I==5 .AND. J==5 .AND. K==4) THEN
                 !    WRITE(*,*) 'VP=',VP,'OMZP=',OMZP,'VM=',VM,'OMZM=',OMZM,'OMZ(I,J-1,K)=',OMZ(I,J-1,K)
                 !    WRITE(*,*)'MY_RANK=',MY_RANK,':IC=',IC,'IEYP=',IEYP,'EDGE(IEYP)%OMEGA(-1)=',EDGE(IEYP)%OMEGA(-1),'WP=',WP,'OMYP=',OMYP,'WOMY=',WOMY,'VOMZ=',VOMZ,'VTRM=',VTRM,',FVX(',I,',',J,',',K,') = ',FVX(I,J,K)
                 !END IF
              ENDDO
           ENDDO
        ENDDO
