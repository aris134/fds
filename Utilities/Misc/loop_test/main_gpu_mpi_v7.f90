PROGRAM LOOP3D
    ...
    ! V7: Uses GPU-aware MPI to pass device allocations directly to the MPI calls
    USE OMP_LIB
    IMPLICIT NONE
    include 'mpif.h'
    ! MPI declarations
...
TYPE(EDGE_TYPE), ALLOCATABLE, DIMENSION(:) :: EDGE
    ...
    IF (MY_RANK==0) THEN
        WRITE(FILENAME,'(A,I4,A,I2,A,I4,A)') 'loop3d_',IBAR,'GRID_',NT,'THR_',INT(NUM_TASKS),'RANKS_GPU_MPI_V7.txt'
    ...
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:U_SEND)
    ALLOCATE(U_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:V_SEND)
    ALLOCATE(V_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:W_SEND)
    ALLOCATE(W_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:U_RECV)
    ALLOCATE(U_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:V_RECV)
    ALLOCATE(V_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:W_RECV)
    ALLOCATE(W_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMX_SEND)
    ALLOCATE(OMX_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMY_SEND)
    ALLOCATE(OMY_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMZ_SEND)
    ALLOCATE(OMZ_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TXY_SEND)
    ALLOCATE(TXY_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TXZ_SEND)
    ALLOCATE(TXZ_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TYZ_SEND)
    ALLOCATE(TYZ_SEND(TOTAL_SEND_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMX_RECV)
    ALLOCATE(OMX_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMY_RECV)
    ALLOCATE(OMY_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:OMZ_RECV)
    ALLOCATE(OMZ_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TXY_RECV)
    ALLOCATE(TXY_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TXZ_RECV)
    ALLOCATE(TXZ_RECV(TOTAL_RECV_PTS))
    !$OMP ALLOCATORS ALLOCATE(OMP_TARGET_DEVICE_MEM_ALLOC:TYZ_RECV)
    ...
    SUBROUTINE EXCHANGE_TAU_OMEGA(NUM_NBRS,NBR_EXT_BBOXES,HALO_SEND_SIZES,HALO_SEND_DISPLS,HALO_RECV_SIZES,&
        ...
        !$OMP HAS_DEVICE_ADDR(OMX_SEND,OMY_SEND,OMZ_SEND,TXZ_SEND,TXY_SEND,TYZ_SEND)
        ...
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO NUM_THREADS(128) &
        !$OMP HAS_DEVICE_ADDR(OMX_RECV,OMY_RECV,OMZ_RECV,TXZ_RECV,TXY_RECV,TYZ_RECV)
    ...
    SUBROUTINE EXCHANGE_VELOCITY(NUM_NBRS,NBR_EXT_BBOXES,HALO_SEND_SIZES,HALO_SEND_DISPLS,HALO_RECV_SIZES,&
        ...
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO NUM_THREADS(128) HAS_DEVICE_ADDR(U_SEND,V_SEND,W_SEND)
        ...
        !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO NUM_THREADS(128) HAS_DEVICE_ADDR(U_RECV,V_RECV,W_RECV)
    ...
    SUBROUTINE EXCHANGE_DOUBLE_DATA(SEND_DATA, RECV_DATA, NUM_NBRS,SEND_SIZES,SEND_DISPLS,&
        ...
        DO RK=1,NUM_NBRS
            ...
            !$OMP TARGET DATA USE_DEVICE_ADDR(RECV_DATA)
        ...
        DO RK=1,NUM_NBRS
            NBR_RANK = NBR_RANKS(RK)
            !$OMP TARGET DATA USE_DEVICE_ADDR(SEND_DATA)
