PROGRAM LOOP3D
    ...
    ! V6: Swap out OMEGA and TAU data members from static arrays to dynamic allocations
    ...
    TYPE EDGE_TYPE
       ...
       REAL(EB), ALLOCATABLE, DIMENSION (:) :: OMEGA
       REAL(EB), ALLOCATABLE, DIMENSION (:) :: TAU
    END TYPE EDGE_TYPE
    ! define custom mapper
    !$OMP DECLARE MAPPER(EDGE_MAPPER : EDGE_TYPE :: VAR) MAP (TO: VAR%OMEGA(-2:2),VAR%TAU(-2:2))
    ...
    WRITE(FILENAME,'(A,I3,A,I3,A,I3,A)') 'loop3d_',IBAR,'GRID_',NT,'THR_',NUM_TIME_STEPS,'STEPS_GPU_V6.txt'
    ...
    DO IE=0,MAX_EDGE
       ALLOCATE(EDGE(IE)%OMEGA(-2:2))
       ALLOCATE(EDGE(IE)%TAU(-2:2))
       EDGE(IE)%OMEGA(:) = -1.E6_EB
       EDGE(IE)%TAU(:) = -1.E6_EB
    END DO
    DO IE=1,MAX_EDGE,2
    ...
    !$OMP MAP(TO:CELL_INDEX(0:IBP1,0:JBP1,0:KBP1)) MAP(TO:CELL(0:IC)) MAP(MAPPER(EDGE_MAPPER), TO:EDGE(0:MAX_EDGE))   &
