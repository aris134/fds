PROGRAM LOOP3D
    ...
    WRITE(FILENAME,'(A,I3,A,I3,A,I3,A)') 'loop3d_',IBAR,'GRID_',NT,'THR_',NUM_TIME_STEPS,'STEPS_GPU_V5.txt'
    ...
    DO K=1,KBAR
       DO J=1,JBAR
          DO I=1,IBAR
             IF( .NOT. (ANY( K==(/1,KBAR/) ) .OR. ANY( J==(/1,JBAR/) ) .OR. ANY( I==(/1,IBAR/) )) ) CYCLE
             IC = IC + 1
             CELL(IC + 0 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 1
             CELL(IC + 1 * ICMAX) = CELL_INDEX(I+1,J  ,K  ) + 1
             CELL(IC + 2 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 2
             CELL(IC + 3 * ICMAX) = CELL_INDEX(I  ,J+1,K  ) + 2
             CELL(IC + 4 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 3
             CELL(IC + 5 * ICMAX) = CELL_INDEX(I  ,J  ,K-1) + 3
             CELL(IC + 6 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 4
             CELL(IC + 7 * ICMAX) = CELL_INDEX(I  ,J+1,K  ) + 4
             CELL(IC + 8 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 5
             CELL(IC + 9 * ICMAX) = CELL_INDEX(I  ,J  ,K-1) + 5
             CELL(IC + 10 * ICMAX) = CELL_INDEX(I  ,J  ,K  ) + 6
             CELL(IC + 11 * ICMAX) = CELL_INDEX(I+1,J  ,K  ) + 6
             DO IE=1,NEDGE
                MAX_EDGE = MAX(MAX_EDGE,CELL(IC + (IE-1) * ICMAX))
             ENDDO
          ENDDO
       ENDDO
    ENDDO
    ...
    ALLOCATE(OMEGA_ARR(0:4*MAX_EDGE_P1))
    ALLOCATE(TAU_ARR(0:4*MAX_EDGE_P1))

    OMEGA_ARR(0:4*MAX_EDGE_P1) = -1.E6_EB
    TAU_ARR(0:4*MAX_EDGE_P1) = -1.E6_EB

    DO IE=1,MAX_EDGE,2
       DO I=0,3
           OMEGA_ARR(IE + I * MAX_EDGE_P1) = 1.5E-4_EB
           TAU_ARR(IE + I * MAX_EDGE_P1) = 2.5E-4_EB
       END DO
    ENDDO
    ...
    !$OMP MAP(TO:CELL_INDEX(0:IBP1,0:JBP1,0:KBP1)) MAP(TO:CELL(0:NEDGE*ICMAX)) MAP(TO:OMEGA_ARR(0:4*MAX_EDGE_P1))                 &
    !$OMP MAP(TO:TAU_ARR(0:4*MAX_EDGE_P1)) MAP(TO:RHOP(0:IBP1,0:JBP1,0:KBP1)) MAP(TO:RDX(0:IBAR)) MAP(TO:RDY(0:JBAR))             &
       ...
       DO K=1,KBAR
          DO J=1,JBAR
             DO I=0,IBAR
                ...
                TXYM  = TXY(I,J-1,K)
                IC    = CELL_INDEX(I,J,K)
                IEYP = CELL(IC + 7 * ICMAX)
                IEYM = CELL(IC + 5 * ICMAX)
                IEZP = CELL(IC + 11 * ICMAX)
                IEZM = CELL(IC + 9 * ICMAX)
                IF (OMEGA_ARR(IEYP)>-1.E5_EB) THEN
                   OMYP = OMEGA_ARR(IEYP)
                   TXZP = TAU_ARR(IEYP)
                ENDIF
                IF (OMEGA_ARR(IEYM + 1 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMYM = OMEGA_ARR(IEYM + 1 * MAX_EDGE_P1)
                   TXZM = TAU_ARR(IEYM + 1 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEZP + 2 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMZP = OMEGA_ARR(IEZP + 2 * MAX_EDGE_P1)
                   TXYP = TAU_ARR(IEZP + 2 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEZM + 3 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMZM = OMEGA_ARR(IEZM + 3 * MAX_EDGE_P1)
                   TXYM = TAU_ARR(IEZM + 3 * MAX_EDGE_P1)
                ENDIF
       ...
       DO K=1,KBAR
          DO J=0,JBAR
             DO I=1,IBAR
                ...
                TXYM  = TXY(I-1,J,K)
                IC    = CELL_INDEX(I,J,K)
                IEXP = CELL(IC + 3 * ICMAX)
                IEXM = CELL(IC + 1 * ICMAX)
                IEZP = CELL(IC + 11 * ICMAX)
                IEZM = CELL(IC + 10 * ICMAX)
                IF (OMEGA_ARR(IEXP + 2 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMXP = OMEGA_ARR(IEXP + 2 * MAX_EDGE_P1)
                   TYZP = TAU_ARR(IEXP + 2 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEXM + 3 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMXM = OMEGA_ARR(IEXM + 3 * MAX_EDGE_P1)
                   TYZM = TAU_ARR(IEXM + 3 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEZP)>-1.E5_EB) THEN
                   OMZP = OMEGA_ARR(IEZP)
                   TXYP = TAU_ARR(IEZP)
                ENDIF
                IF (OMEGA_ARR(IEZM + 1 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMZM = OMEGA_ARR(IEZM + 1 * MAX_EDGE_P1)
                   TXYM = TAU_ARR(IEZM + 1 * MAX_EDGE_P1)
                ENDIF
       ...
       DO K=0,KBAR
          DO J=1,JBAR
             DO I=1,IBAR
                ...
                IC    = CELL_INDEX(I,J,K)
                IEXP = CELL(IC + 3 * ICMAX)
                IEXM = CELL(IC + 2 * ICMAX)
                IEYP = CELL(IC + 7 * ICMAX)
                IEYM = CELL(IC + 6 * ICMAX)
                IF (OMEGA_ARR(IEXP)>-1.E5_EB) THEN
                   OMXP = OMEGA_ARR(IEXP)
                   TYZP = TAU_ARR(IEXP)
                ENDIF
                IF (OMEGA_ARR(IEXM + 1 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMXM = OMEGA_ARR(IEXM + 1 * MAX_EDGE_P1)
                   TYZM = TAU_ARR(IEXM + 1 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEYP + 2 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMYP = OMEGA_ARR(IEYP + 2 * MAX_EDGE_P1)
                   TXZP = TAU_ARR(IEYP + 2 * MAX_EDGE_P1)
                ENDIF
                IF (OMEGA_ARR(IEYM + 3 * MAX_EDGE_P1)>-1.E5_EB) THEN
                   OMYM = OMEGA_ARR(IEYM + 3 * MAX_EDGE_P1)
                   TXZM = TAU_ARR(IEYM + 3 * MAX_EDGE_P1)
                ENDIF
